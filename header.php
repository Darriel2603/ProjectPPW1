...
include("admin/include/CSRF.php");
...
$i=1;
$statement = $pdo->prepare("SELECT * FROM language");
$statement->execute();
...
$statement = $pdo->prepare("SELECT * FROM payment WHERE payment_status=?");
...
foreach ($result as $row) {
	...
	if($time>24) {
		...
		foreach ($result1 as $row1) {
			$statement2 = $pdo->prepare("SELECT * FROM product WHERE p_id=?");
			...
			$statement = $pdo->prepare("UPDATE product SET p_qty=? WHERE p_id=?");
			$statement->execute(array($final,$row1['product_id']));
		}
		
		// Deleting data from table
		$statement1 = $pdo->prepare("DELETE FROM order_table WHERE payment_id=?");
		$statement1->execute(array($row['payment_id']));

		$statement1 = $pdo->prepare("DELETE FROM payment WHERE id=?");
...
<html lang="en">
    ...
    <link rel="icon" type="image/png" href="assets/img/<?php echo $favicon; ?>">
    ...
    <?php if($cur_page == 'blog-single.php'): ?>
		...
		<meta property="og:image" content="assets/content/<?php echo $og_photo; ?>">
	<?php endif; ?>
                ...
                <div class="col-md-5 right">
                    <ul>
                        ...
                        if(isset($_SESSION['customer'])) {
                            ...
                            <li><a href="dashboard.php"><i class="fa-sharp fa-solid fa-house"></i><?php echo LANG_VALUE_89; ?></a></li>
                            <?php
                        } else {
                            ?>
                            <li><a href="login.php"><i class="fa fa-sign-in"></i> <?php echo LANG_VALUE_9; ?></a></li>
                            <li><a href="registration.php"><i class="fa-sharp fa-solid fa-user-plus"></i> <?php echo LANG_VALUE_15; ?></a></li>
                            <?php	
                        }
                        ?>

                        <li><a href="cart.php"><i class="fa-sharp fa-solid fa-cart-shopping"></i> <?php echo LANG_VALUE_18; ?> (<?php echo LANG_VALUE_1; ?><?php
