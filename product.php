...
$statement = $pdo->prepare("UPDATE product SET p_total_view=? WHERE p_id=?");
...
if(isset($_POST['form_review'])) {
    $statement = $pdo->prepare("SELECT * FROM rating where p_id=? AND cust_id = ?");
    ...
    if($total) {
        ...
        $statement = $pdo->prepare("INSERT INTO rating (p_id,cust_id,comment,rating) VALUES (?,?,?,?)");
        $statement->execute(array($_REQUEST['id'],$_SESSION['customer']['cust_id'],$_POST['comment'],$_POST['rating']));
        ...
        if($added == 1) {
           $error_message1 = 'This product is already added to the shopping cart.';
            ...
            if(isset($_POST['size_id'])) {
                ...
                $statement = $pdo->prepare("SELECT * FROM size WHERE size_id=?");
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-12 mb_20">
                                            ...
                                            <select name="size_id" class="form-control select2" style="width:auto;">
                                                <?php 
                                                $statement = $pdo->prepare("SELECT * FROM size");
                                                $statement ->execute();
                                                $result = $statement->fetchAll(PDO::FETCH_ASSOC);
                                                foreach ($result as $row) {
